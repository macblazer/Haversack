...
public actor EphemeralKeychain: KeychainImplementation {
    ...
    nonisolated public static let errorDomain = "haversack.unit_testing.mock"

    public init() { }

    public func setMockData(_ newMockData: [String: Any]) {
    ...
    public func searchForOne<T: KeychainQuerying>(_ querying: T) throws -> T.Entity {
    ...
    /// > Tip: First create your query and run the code, then look at the `localizedDescription` of the thrown error to
    /// find the key that must be populated in the ``mockData``.
    public func search<T: KeychainQuerying>(_ querying: T) throws -> [T.Entity] {
        let theKey = key(for: querying)
    ...
    public func save(_ item: SecurityFrameworkQuery) throws {
        ...
        print("Kyle - We are in the save function with theKey: \(theKey)")

        if mockData[theKey] == nil {
            print("Kyle - We are writing to the dictionary with item: \(item)")
            print("Kyle - dictionary is %x", mockData)
    ...
    public func importItems<EntityType: KeychainImportable>(
        _ items: Data,
    ...
    nonisolated func key<T: KeychainQuerying>(for querying: T) -> String {
        return key(for: querying.query)
    }

    nonisolated func key(for query: SecurityFrameworkQuery) -> String {
